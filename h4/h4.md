...
Tässä harjoituksessa käytettiin rekursiota. Eli jos ffuf löytää alihakemiston, käy se myös sen läpi kokonaan. Tässä esimerkissä ffuf löytää aluksi alihakemiston `admin`. Tämän jälkeen rekursion avulla ffuf käy wordlistan uudestaan läpi `admin` alihakemistosta. Ffuf löysi polun `admin/users`. Ffuf käy vielä tämän läpi ja löytää `admin/users/96`. Ffuf käy tämänkin alihakemiston läpi, mutta ei löydä pidemmälle. 
...
Tässä meinasin joutua taas samaan ansaan, eli copy paste warrior. Malliesimerkissä fuzzataan ``ffuf.test``, joten se piti vaihtaa `localhost` jotta fuzzaus onnistuu.
